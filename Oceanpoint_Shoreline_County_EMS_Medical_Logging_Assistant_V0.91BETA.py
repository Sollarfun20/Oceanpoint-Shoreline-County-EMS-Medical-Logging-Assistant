...
#Prototype Version V0.9BETA, internal usage ONLY
#Python 3.9 (64-bit)
...
#Prints time left on timer and clears current input
def TimerPrint(time):
    global CurrentInput
    print(time)
    CurrentInput = "Cleared"
...
#Outputs main string for EMT level of CPR protocol, and queries levelthree command for strings if applicable
def CPRCareLevelOne():
     global ListOffset
...
def AirwayCareLevelThree(iteration):
    global ListOffset
    if ProviderTrainingLevel == 3:
        if iteration == 0:
            return f"{Style.RESET_ALL}{Fore.RED}PARAMEDIC{Style.RESET_ALL}"
...
**Other Notes (Ex. Units had to stage before entry):** {ReportNotes}"""
    ...
    #COMMAND LIST command, lists all commands
            ...
            Sets a 2 minute timer for AED rythym analyzation. Will play a sound and prompt on display when timer is done. Enter "cancel" to cancel and "timeleft" to see the time remaining.
